...
const hubRepo = "fluctio-ai/fluctio"
...
func pluginInstallCmd() *cobra.Command {
	return &cobra.Command{
		...
		Short: "Install a plugin from Fluctio Hub, GitHub, npm, or local path",
		Long: `Install a plugin. The source is auto-detected:

  fluctio plugins install telegram                        # Fluctio Hub
  fluctio plugins install github.com/user/repo            # GitHub repo
  fluctio plugins install @ollama/web-search              # npm plugin (bridged)
  fluctio plugins install ./my-plugin                     # local directory`,
...
func installFromGitHub(source, pluginsDir string) error {
	...
	// Clone to temp dir
	tmpDir, err := os.MkdirTemp("", "fluctio-plugin-*")
	if err != nil {
		return err
	}
...
func installFromHub(name, pluginsDir string) error {
	...
	fmt.Printf("Installing %q from Fluctio Hub...\n", name)
	...
	// Find top-level dir (name varies: fluctio-main, fluctio-v0.16.0, etc.)
	...
	if _, err := os.Stat(pluginDir); os.IsNotExist(err) {
		return fmt.Errorf("plugin %q not found in Fluctio Hub", name)
	}
...
func installFromNpm(pkg, pluginsDir string) error {
	...
	pluginID = strings.TrimPrefix(pluginID, "fluctio-")

	// 1. npm install to temp dir to inspect the package
	tmpDir, err := os.MkdirTemp("", "fluctio-npm-*")
	if err != nil {
		return err
	}
	defer os.RemoveAll(tmpDir)
	...
	// 2. Check if it's a compatible plugin (supports fluctio or openclaw plugin format)
	pkgDir := filepath.Join(tmpDir, "node_modules", pkg)
	isPlugin := false
	for _, marker := range []string{"fluctio.plugin.json", "openclaw.plugin.json"} {
		if _, err := os.Stat(filepath.Join(pkgDir, marker)); err == nil {
			isPlugin = true
			break
		}
	}
	// Also check package.json for fluctio/openclaw field
	if !isPlugin {
		if data, err := os.ReadFile(filepath.Join(pkgDir, "package.json")); err == nil {
			var pj map[string]json.RawMessage
			if json.Unmarshal(data, &pj) == nil {
				for _, key := range []string{"fluctio", "openclaw"} {
					if _, ok := pj[key]; ok {
						isPlugin = true
						break
					}
	...
	if _, err := os.Stat(proxyJS); os.IsNotExist(err) {
		fmt.Println("Installing plugin-bridge from Fluctio Hub...")
...
`
	...
	if testErr != nil && toolCount == 0 {
		if hasChannel {
			return fmt.Errorf("cannot install %s: this is a channel plugin that requires a separate runtime. Consider writing a native Fluctio plugin instead", pkg)
		}
		return fmt.Errorf("cannot install %s: plugin is not compatible with Fluctio bridge", pkg)
	}
