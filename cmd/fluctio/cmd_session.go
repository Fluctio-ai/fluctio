...
from the LOCAL fluctio.db — "online" sessions need their DB pulled
down first (rsync ~/.fluctio/fluctio.db from the host).

Examples:
  fluctio session export http://localhost:18953/agents/agt_xxx/chat/s-yyy/
  fluctio session export https://app.fluctio.ai/agents/agt_xxx/chat/s-yyy -o /tmp/run.json`,
	...
	cmd.Flags().StringVarP(&outputPath, "output", "o", "",
		"output JSON path (default ~/.fluctio/logs/<sessionKey>.json)")
	cmd.Flags().StringVar(&dbPath, "db", "",
		"sqlite DB path (default ~/.fluctio/fluctio.db)")
	return cmd
}
...
// dbPath defaults to ~/.fluctio/fluctio.db with the same pragma
...
func openStoreAt(dbPath string) (*store.DBStore, func(), error) {
	if dbPath == "" {
		...
		dbPath = filepath.Join(home, ".fluctio", "fluctio.db")
	}
...
// ~/.fluctio/logs/<sessionKey>.json. Per-session filename so a
...
func resolveOutputPath(explicit, sessionKey string) (string, error) {
	...
	return filepath.Join(home, ".fluctio", "logs", sessionKey+".json"), nil
}
