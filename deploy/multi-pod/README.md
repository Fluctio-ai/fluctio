...
- **MinIO** — S3-compatible bucket for workspace artifacts (auto-creates `fluctio` bucket)
- **pod-a** on `:18953` and **pod-b** on `:18954` — identical gateway binaries, pointed at the same DB and S3

Both pods use `FLUCTIO_AUTH_TOKEN=dev-admin-token`. Any admin API call takes `Authorization: Bearer dev-admin-token`.
...
# Browse bucket "fluctio" and expect keys like:
#   <agent-id>/<filename>
```
...
- **Sandbox lifecycle** — lazy creation + idle eviction + flush is in code
  ...
  API key. Set `FLUCTIO_SANDBOX_BACKEND=docker` or `e2b` plus an `E2B_API_KEY`
  to test.
