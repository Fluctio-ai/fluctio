...
- fluctio -> plugin:  initialize, hook.register, hook.fire (notification).
- plugin -> fluctio:  result for the synchronous calls, plus chat.send
...
def handle_hook_register(params, req_id):
    # Tell fluctio we want to be notified on post_turn. The server will
...
def handle_hook_fire(params):
    ...
    # Push a fixed follow-up message into the same chat. fluctio's
