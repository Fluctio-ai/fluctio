# Fluctio API Integration

Use this skill when integrating an upstream application with Fluctio as an
agent runtime.

The canonical reference is `docs/upstream-api.md` in the Fluctio repository.
...
Use `/api/*` or the `fluctio` CLI only for operator/admin workflows such as
...
Do not expose Fluctio API keys to browsers or mobile clients. Route calls
...
Authorization: Bearer <FLUCTIO_API_KEY>
Content-Type: application/json
X-Fluctio-Session-Key: <deterministic-session-key>
```
...
- `agent_id` selects the Fluctio agent. Body wins over
  `X-Fluctio-Agent-ID`.
- `user` is the upstream stable user ID. Body wins over
  `X-Fluctio-End-User`.
- `X-Fluctio-Session-Key` controls conversation history. Use a deterministic
...
Or skip provisioning and pass `user` on every chat call. Fluctio will lazy
...
1. Add server-side Fluctio client configuration:
   - base URL
   - API key
   - agent ID
2. Add or reuse upstream conversation IDs.
3. Map upstream user IDs to Fluctio `user` or `/v1/users`.
4. Implement streaming SSE parsing for `/v1/chat/completions`.
5. Persist or derive `X-Fluctio-Session-Key` per conversation.
...
- Do not call dashboard `/api/chat/stream` for upstream app chat unless you are
  embedding Fluctio's own dashboard semantics.
- Do not put Fluctio API keys in frontend code.
- Do not use email/display name as the stable Fluctio `user` value.
