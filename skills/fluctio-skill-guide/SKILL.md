---
name: fluctio-skill-guide
description: Create new skills for Fluctio agents. Use when the user asks to create a skill, turn a workflow into a skill, or build reusable automation. Also use when discussing skill format, structure, or best practices.
---

# Fluctio Skill Guide

Guide for creating new skills that Fluctio agents can discover and use.
...
| `metadata.fluctio.emoji` | No | Display icon |
| `metadata.fluctio.always` | No | If true, full content is always in system prompt |
| `metadata.fluctio.os` | No | OS requirements (darwin, linux, windows) |
| `metadata.fluctio.requires.bins` | No | All listed binaries must exist on PATH |
| `metadata.fluctio.requires.anyBins` | No | At least one must exist |
| `metadata.fluctio.requires.env` | No | Required environment variables |
| `metadata.fluctio.primaryEnv` | No | Maps config apiKey to this env var |
...
4. **Save** — Write to `{agentDir}/skills/{skill-name}/SKILL.md` for agent-level, or `~/.fluctio/skills/{skill-name}/SKILL.md` for user-level
