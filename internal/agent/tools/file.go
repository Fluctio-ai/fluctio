...
// admin-managed global skills directory (~/.fluctio/skills/). Reads are
...
// agent on the host.
const globalSkillsDirSuffix = "/.fluctio/skills"

// errGlobalSkillsDirWrite is returned when write_file targets
// ~/.fluctio/skills/ from inside an agent chat. The message tells the model
// exactly how to recover.
var errGlobalSkillsDirWrite = fmt.Errorf("access denied: ~/.fluctio/skills/ is the admin-managed global skills directory. To create a new skill, load the \"skill-creator\" skill and follow its workflow (it scaffolds into this agent's private skills dir). To install an existing one, use the install_skill tool")
...
// when it falls under one of the Fluctio-managed roots
// (~/.fluctio/...) — those are runtime internals and should keep
...
func hostHomePath(path string) (string, bool) {
	...
	if path == "~" || strings.HasPrefix(path, "~/") {
		// Sandbox-only / Fluctio-internal subtrees: skip host expansion
		...
		//   ~/.fluctio/... — runtime internals (db, workspaces, …)
		//   ~/.agents/...   — sandbox bind-mount target for npx skills.
		//                     Host has these at <FLUCTIO_HOME>/users/<uid>/skills/,
		...
		if strings.HasPrefix(path, "~/.fluctio") || strings.HasPrefix(path, "~/.agents") {
			return "", false
		}
	...
	if strings.HasPrefix(path, "/Users/") || strings.HasPrefix(path, "/home/") {
		// Refuse Fluctio-internal subpaths even when the chatter
		// reaches them via the host-home channel. Same guard as
		// errGlobalSkillsDirWrite, broader scope.
		if home, err := os.UserHomeDir(); err == nil {
			fluctioDir := filepath.Join(home, ".fluctio")
			if path == fluctioDir || strings.HasPrefix(path, fluctioDir+string(filepath.Separator)) {
				return "", false
			}
		}
		return path, true
...
// chat-time skill writes should land in — always the agent home
// (skills/ writes route to the agent layer: agents/<aid>/agent/skills/).
func (r *Registry) skillRoot() string {
	return r.systemRoot
}

// skillStoreOwner returns the workspace.Store pseudo-owner key the
// chat-created skill should mirror to — always the agent ID (skills
// live in the agent layer now).
...
//     AND for `skills/...` writes — chat-created skills land in the agent
//     layer (agents/<aid>/agent/skills/), so the leading `skills/` prefix
//     resolves against agent home, where SkillsLoader's scan picks it up;
...
func (r *Registry) rootForPath(path string) string {
	...
	clean := filepath.Clean(path)
	if clean == "skills" || strings.HasPrefix(clean, "skills"+string(filepath.Separator)) {
		// `skills/...` writes land in the agent layer (agent home +
		// "skills/"). The leading `skills/` prefix is preserved so
		// SkillsLoader's scan picks it up next turn.
		return r.systemRoot
	}
...
// admin-managed ~/.fluctio/skills/ directory. Works across user home
...
func makeReadFile(r *Registry) ToolFunc {
	return func(ctx context.Context, rawArgs json.RawMessage) (string, error) {
		...
		if r.systemFileStore != nil && r.agentID != "" && basenameIsSystemFile(args.Path) {
			...
			// metadata dir (e.g. ~/.fluctio/agents/<id>/agent) which
...
func registerSandboxedFile(r *Registry, ex sandbox.Executor) {
	r.Register("read_file", "Read the contents of a file", map[string]interface{}{
		...
		// /data/.fluctio/workspaces/<id>/IDENTITY.md still hits the DB.
		if r.systemFileStore != nil && r.agentID != "" && basenameIsSystemFile(args.Path) {
			name := filepath.Base(filepath.Clean(args.Path))
			if data, err := r.readSystemFileForUser(ctx, r.systemFileUserID(name), name); err == nil {
				return truncateReadFileResult(args.Path, data), nil
