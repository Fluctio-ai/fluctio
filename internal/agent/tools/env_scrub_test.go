...
func TestIsSensitiveEnvKey(t *testing.T) {
	cases := []struct {
		...
		{"FLUCTIO_STORAGE_DSN", true},
		{"FLUCTIO_OBJECT_STORE_ACCESSKEY", true},
		{"FLUCTIO_OBJECT_STORE_SECRETKEY", true},
		{"FLUCTIO_OBJECT_STORE_BUCKET", true},
		{"FLUCTIO_OBJECT_STORE_ALIYUN_INTERNAL", true},
		{"FLUCTIO_SANDBOX_BOXLITE_URL", true},
		...
		{"FLUCTIO_HOME", false},
		{"FLUCTIO_LOG_LEVEL", false},
		{"FLUCTIO_DEPLOY", false},
		{"FLUCTIO_ALLOW_HOST_EXEC", false},
	}
	for _, c := range cases {
...
func TestScrubSensitiveEnv(t *testing.T) {
	in := []string{
		"PATH=/usr/bin",
		"HOME=/home/x",
		"FLUCTIO_STORAGE_DSN=postgres://user:pw@host/db",
		"FLUCTIO_OBJECT_STORE_ACCESSKEY=AKIAEXAMPLE",
		"FLUCTIO_OBJECT_STORE_SECRETKEY=secret",
		"ANTHROPIC_API_KEY=sk-ant-x",
		"AWS_ACCESS_KEY_ID=AKIA",
		"FLUCTIO_HOME=/var/lib/fluctio",
		"LANG=en_US.UTF-8",
...
func TestBuildSubprocessEnvOverridesSkillKeys(t *testing.T) {
	...
	t.Setenv("FLUCTIO_STORAGE_DSN", "must-be-stripped")
	...
	for _, kv := range out {
		...
		if len(kv) >= len("FLUCTIO_STORAGE_DSN=") && kv[:len("FLUCTIO_STORAGE_DSN=")] == "FLUCTIO_STORAGE_DSN=" {
			sawDSN = true
		}
	}
	if !sawFalKey {
	...
	if sawDSN {
		t.Errorf("FLUCTIO_STORAGE_DSN must be scrubbed; leaked in %v", out)
	}
}
