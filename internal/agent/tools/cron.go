...
func RegisterCronTools(r *Registry, st store.Store, userID, agentID string) {
	r.Register("create_cron_job",
		"Create a scheduled task. Use this for any user request that names a specific time, an interval, or a recurring schedule (e.g. \"5 分钟后提醒\", \"every Monday 9am\", \"each day at 8\"). When the schedule fires, the agent receives `message` as a fresh inbound prompt. By default it fires on the same channel/chat the request originated from. To push to a DIFFERENT channel (e.g. set the task from QQ but deliver to WeChat): FIRST call list_channels to list chats you can deliver to, THEN copy the ENTIRE (channel, accountId, chatId) triple from the target chat's row into channel/accountId/chatId below — pass all three together, never just one or two, because an incomplete address cannot be delivered to. Do NOT write timed reminders into HEARTBEAT.md — that file is only for conditional self-checks reviewed at every heartbeat tick. If the task is BACKGROUND work that must NOT notify the user (e.g. periodic memory tidy, self-maintenance, log rotation, daily self-review), pass silent=true — the agent runs the scheduled message but delivers no reply to any chat. The default (silent omitted/false) is a normal reminder that DOES reach the user — use that for anything the user asked to be told about (reminders, notifications, daily reports they want to read). When unsure, prefer the default (non-silent): a missed silent flag leaks a background chore to the user, while a missed notification just means the user doesn't get pinged.",
