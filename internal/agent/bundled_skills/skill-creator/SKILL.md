...
## Fluctio runtime — read this first

This skill is invoked from a Fluctio agent. The host runtime has its
...
The methodology below is adapted from an upstream skill-creator;
file-layout details from the upstream (`/tmp/` staging, packaging
`.skill` files) don't apply on Fluctio — persisting a skill here is
just `write_file` calls with the `skills/<name>/` prefix above. The
eval / benchmark / description-optimization scaffolding (now in
`references/`) still applies if the user wants to iterate or measure;
those scripts are read-only helpers, you don't need them to ship a
basic skill.
...
For running test cases, benchmarking, blind comparison, and the
improve-rerun loop, see `references/evaluating.md`.

For optimizing the skill's `description` for better triggering
accuracy (trigger eval queries + the optimization loop), see
...
Please add steps to your TodoList, if you have such a thing, to make
sure you don't forget — especially "Create evals JSON and run
`eval-viewer/generate_review.py` so the human can review test cases".
