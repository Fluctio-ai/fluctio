...
// A condition node is a pure routing point (no leaf) — its outgoing edges'
// `when` expressions decide the branch. Verify it runs as a pass-through and
// the branch edges route correctly (MaxKB-style condition-node mental model).
func TestBranch_ConditionNode(t *testing.T) {
	const def = `
version: 1
nodes:
  - {name: src, kind: tool, tool: t, output: {score: {type: number}}}
  - {name: branch, kind: condition}
  - {name: hi, kind: tool, tool: th}
  - {name: lo, kind: tool, tool: tl}
edges:
  - {from: src, to: branch}
  - {from: branch, to: hi, when: "${src.score} > 0.5"}
  - {from: branch, to: lo, when: "default"}
`
	tools := &fakeTools{out: map[string]string{"t": `{"score":0.9}`, "th": `{}`, "tl": `{}`}}
	res, err := workflow.NewRunner(&fakeLLM{}, tools, newTestStore(t)).Run(context.Background(), mustParse(t, def), nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	// the condition node itself ran as a pass-through (no leaf call).
	bn, ok := res.Snapshot["branch"]
	if !ok {
		t.Fatal("condition node 'branch' missing from snapshot")
	}
	if bn.Status != workflow.StatusSucceeded {
		t.Errorf("condition node status = %s, want succeeded (pass-through)", bn.Status)
	}
	if bn.Kind != workflow.KindCondition {
		t.Errorf("condition node kind = %q, want condition", bn.Kind)
	}
	// score 0.9 → branch's "${src.score} > 0.5" is true → hi.
	if _, ok := res.Snapshot["hi"]; !ok {
		t.Error("hi not reached (condition branch ${src.score}>0.5 should match)")
	}
	if _, ok := res.Snapshot["lo"]; ok {
		t.Error("lo reached but should be skipped (default not taken when hi matched)")
	}
}
