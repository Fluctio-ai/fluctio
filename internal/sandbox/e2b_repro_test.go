...
//   FLUCTIO_AGENT_ID=agt_ccc0add56d30398bcfa1 \
...
func TestE2BReproWithRealWorkspace(t *testing.T) {
	...
	agentID := os.Getenv("FLUCTIO_AGENT_ID")
	if apiKey == "" || template == "" || agentID == "" {
		t.Skip("set E2B_API_KEY, E2B_TEMPLATE, FLUCTIO_AGENT_ID")
	}
	home := os.Getenv("FLUCTIO_HOME")
	if home == "" {
		home = filepath.Join(os.Getenv("HOME"), ".fluctio")
	}
