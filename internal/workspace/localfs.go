...
type LocalFS struct {
	// root is usually ~/.fluctio/workspaces. Objects for agent foo go to
	// <root>/foo/<path>.
	root string
}
...
// (typically ~/.fluctio/workspaces). Exposed so callers that need
// to compute a host path for an external tool — e.g. "open in
// Finder" / shelling out — can join from the same anchor LocalFS
// uses internally without re-deriving it from FLUCTIO_HOME.
