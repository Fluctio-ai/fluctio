...
// (rather than cmd/fluctio/main) so internal/* code — notably the agent
...
// Version is the Fluctio release tag (e.g. "v0.4.2") set by the
...
// IsHostedDeploy reports whether this fluctio process is running in a
// hosted/multi-tenant deployment (cloud) versus a self-hosted single-
// operator install. Driven by the FLUCTIO_DEPLOY env var:
//
//	FLUCTIO_DEPLOY=hosted        → IsHostedDeploy() == true
//	FLUCTIO_DEPLOY=self-hosted   → false
...
// the most common case (developer running fluctio on their own
...
// osDeployVar reads FLUCTIO_DEPLOY and normalizes it. Lowercased so
// casing variations don't silently bypass the hosted flag.
func osDeployVar() string {
	return strings.ToLower(strings.TrimSpace(os.Getenv("FLUCTIO_DEPLOY")))
}
...
// operator has explicitly opted in via FLUCTIO_ALLOW_HOST_EXEC=1
...
// flow, `fluctio upgrade`, `~/Downloads` triage) sets the env var
...
func IsHostExecAllowed() bool {
	...
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FLUCTIO_ALLOW_HOST_EXEC")))
