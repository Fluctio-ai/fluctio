...
// HTTPS POSTs from Feishu's open platform to the fluctio webhook route
...
// fluctio HTTP server. Trade-off: needs a publicly reachable URL.
//
...
func (l *Feishu) HandleWebhook(body []byte) (responseBody []byte, status int, err error) {
	...
	if peek.Encrypt != "" {
		if l.encryptKey == "" {
			return nil, http.StatusBadRequest, errors.New("feishu webhook is encrypted but no encryptKey configured (set 加密策略 → Encrypt Key in fluctio connect dialog, or clear it in feishu console)")
		}
	...
	if env.Type == "url_verification" || env.Header.EventType == "url_verification" {
		...
		// fluctio connect dialog. Constant-time compare on the
		// match to avoid timing leaks on the token.
		if l.verificationToken == "" {
			return nil, http.StatusUnauthorized,
				errors.New("feishu webhook rejected: no verification token configured — set it in the Feishu console and fluctio connect dialog")
		}
