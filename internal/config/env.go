...
// sandbox backend. Read at process start from FLUCTIO_* environment
// variables — there is no config file. Everything user-facing
// (providers, channels, agents) lives in the database.
//
// Set these as `FLUCTIO_<UPPER_SNAKE_CASE>` (or the explicit name in
...
type EnvStorage struct {
	Type        string // FLUCTIO_STORAGE_TYPE  — "sqlite" (default) or "postgres"
	DSN         string // FLUCTIO_STORAGE_DSN   — empty = sqlite at $FLUCTIO_HOME/fluctio.db
	AutoMigrate bool   // FLUCTIO_STORAGE_AUTO_MIGRATE — default true
}

type EnvRedis struct {
	Enabled  bool   // FLUCTIO_REDIS_ENABLED — enable Redis-backed bus + leases
	Addr     string // FLUCTIO_REDIS_ADDR    — host:port, e.g. redis:6379
	Username string // FLUCTIO_REDIS_USERNAME
	Password string // FLUCTIO_REDIS_PASSWORD
	DB       int    // FLUCTIO_REDIS_DB
	Prefix   string // FLUCTIO_REDIS_PREFIX  — key prefix, default "fluctio"
}

type EnvSandbox struct {
	Enabled         bool   // FLUCTIO_SANDBOX_ENABLED
	Backend         string // FLUCTIO_SANDBOX_BACKEND  — "docker", "e2b", or "boxlite"
	Image           string // FLUCTIO_SANDBOX_IMAGE
	E2BKey          string // E2B_API_KEY
	BoxliteURL      string // FLUCTIO_SANDBOX_BOXLITE_URL — full base URL e.g. https://api.boxlite.ai/v1
	BoxliteClientID string // FLUCTIO_SANDBOX_BOXLITE_CLIENT_ID — default "default"
	BoxliteKey      string // BOXLITE_API_KEY — apikey sent as Authorization: Bearer
	BoxlitePrefix   string // FLUCTIO_SANDBOX_BOXLITE_PREFIX — workspace prefix, default "default"
}

type EnvLog struct {
	Level string // FLUCTIO_LOG_LEVEL    — "debug" / "info" / "warn" / "error"
	Debug bool   // FLUCTIO_DEBUG_MODE   — enable verbose debug output (prompt dump, etc.)
}

// LoadEnv reads the bootstrap configuration from FLUCTIO_* environment
...
func LoadEnv() *EnvConfig {
	...
	if v := os.Getenv("FLUCTIO_STORAGE_DSN"); v != "" {
		cfg.Storage.DSN = v
	}
	if v := os.Getenv("FLUCTIO_STORAGE_AUTO_MIGRATE"); v != "" {
		cfg.Storage.AutoMigrate = v == "true" || v == "1"
	}

	if v := os.Getenv("FLUCTIO_REDIS_ENABLED"); v != "" {
		cfg.Redis.Enabled = v == "true" || v == "1"
	}
	if v := os.Getenv("FLUCTIO_REDIS_ADDR"); v != "" {
		cfg.Redis.Addr = v
		cfg.Redis.Enabled = true
	}
	if v := os.Getenv("FLUCTIO_REDIS_USERNAME"); v != "" {
		cfg.Redis.Username = v
	}
	if v := os.Getenv("FLUCTIO_REDIS_PASSWORD"); v != "" {
		cfg.Redis.Password = v
	}
	if v := os.Getenv("FLUCTIO_REDIS_DB"); v != "" {
		if n, err := strconv.Atoi(v); err == nil {
			cfg.Redis.DB = n
		}
	}
	if v := os.Getenv("FLUCTIO_REDIS_PREFIX"); v != "" {
		cfg.Redis.Prefix = v
	}

	if v := os.Getenv("FLUCTIO_SANDBOX_ENABLED"); v != "" {
		cfg.Sandbox.Enabled = v == "true" || v == "1"
	}
	if v := os.Getenv("FLUCTIO_SANDBOX_BACKEND"); v != "" {
	...
	if v := os.Getenv("FLUCTIO_SANDBOX_IMAGE"); v != "" {
		cfg.Sandbox.Image = v
	}
	if v := os.Getenv("E2B_API_KEY"); v != "" {
		cfg.Sandbox.E2BKey = v
	}
	if v := os.Getenv("FLUCTIO_SANDBOX_BOXLITE_URL"); v != "" {
		cfg.Sandbox.BoxliteURL = v
	}
	if v := os.Getenv("FLUCTIO_SANDBOX_BOXLITE_CLIENT_ID"); v != "" {
		cfg.Sandbox.BoxliteClientID = v
	}
	if v := os.Getenv("BOXLITE_API_KEY"); v != "" {
		cfg.Sandbox.BoxliteKey = v
	}
	if v := os.Getenv("FLUCTIO_SANDBOX_BOXLITE_PREFIX"); v != "" {
		cfg.Sandbox.BoxlitePrefix = v
	}

	if v := os.Getenv("FLUCTIO_LOG_LEVEL"); v != "" {
		cfg.Log.Level = v
	}
	if v := os.Getenv("FLUCTIO_DEBUG_MODE"); v == "true" || v == "1" {
		cfg.Log.Debug = true
	}
	return cfg
}

// applyObjectStoreEnv reads FLUCTIO_OBJECT_STORE_* env vars into cfg.
func applyObjectStoreEnv(cfg *Config) {
	read := func(key string) string { return os.Getenv("FLUCTIO_OBJECT_STORE_" + key) }
	if v := read("TYPE"); v != "" {
...
var debugMode = os.Getenv("FLUCTIO_DEBUG_MODE") == "true" || os.Getenv("FLUCTIO_DEBUG_MODE") == "1"

// DebugMode returns true when FLUCTIO_DEBUG_MODE=true|1. Use this to
...
func ScrubBootSecrets() {
	keys := []string{
		"FLUCTIO_STORAGE_DSN",
		"FLUCTIO_OBJECT_STORE_TYPE",
		"FLUCTIO_OBJECT_STORE_LOCAL_ROOT",
		"FLUCTIO_OBJECT_STORE_REGION",
		"FLUCTIO_OBJECT_STORE_BUCKET",
		"FLUCTIO_OBJECT_STORE_PREFIX",
		"FLUCTIO_OBJECT_STORE_ACCESSKEY",
		"FLUCTIO_OBJECT_STORE_SECRETKEY",
		"FLUCTIO_OBJECT_STORE_ACCOUNTID",
		"FLUCTIO_OBJECT_STORE_ENDPOINT",
		"FLUCTIO_OBJECT_STORE_USESSL",
		"FLUCTIO_OBJECT_STORE_ALIYUN_INTERNAL",
		"FLUCTIO_REDIS_PASSWORD",
		"BOXLITE_API_KEY",
		"E2B_API_KEY",
	}
	for _, k := range keys {
...
// by gateway boot to layer FLUCTIO_OBJECT_STORE_* on top of the DB-
